...
function App() {
  ...
  const createPeer = (targetId: string) => {
    ...
    // 1. ADD TRACKS FIRST: This ensures the SDP (Offer/Answer) 
    // contains the media information for the return stream.
    if (localStreamRef.current) {
  ...
  // 2. Caller Logic (User 1)
  // const callUser = async () => {
  //   if (!selectedUser) return;

  //   // Ensure local stream is active before calling
  //   if (!localStreamRef.current) {
  ...
  const acceptCall = async () => {
    ...
    // 2. Create and set local answer (Now includes tracks because we added them in createPeer)
    const answer = await pc.createAnswer();
    await pc.setLocalDescription(answer);

    socket.emit("answer-call", { to: incomingCall.from, answer });

    // 3. Process candidates that arrived while we were setting up
    while (pendingCandidates.current.length > 0) {
      const candidate = pendingCandidates.current.shift();
      if (candidate) await pc.addIceCandidate(new RTCIceCandidate(candidate)).catch(e => console.error(e));
    }
